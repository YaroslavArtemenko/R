fundamentalMatrix <- function(T,vector) {
  ...
  cat("Каноническая матрица\n")
  print(canonicalMatrix$matrix)
  
  N = solve(diag(m)-Q)
  cat("Фундаментальная матрица\n")
  print(N)
  
  cat("Вероятность поглощения\n")
  print(N%*%R)
  cat("Относительное время нахождения в заданом состоянии \n")
  print(vector3%*%N)
  cat("Среднее время поглащения \n")
